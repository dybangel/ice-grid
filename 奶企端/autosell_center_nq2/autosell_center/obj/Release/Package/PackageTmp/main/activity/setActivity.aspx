<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="setActivity.aspx.cs" Inherits="autosell_center.main.activity.setActivity" %>
...
.btn:hover {
    ...
    //初始化树的参数
        ...
        function GetDatas() {
            ...
            //	    $.getJSON("GetDatesHandler.ashx", function (json) {
            //	        //alert("JSON Data: " + json[0]["name"]);
            //	        zTreeObj = $.fn.zTree.init($("#tree"), setting, json);
            //	    });

            $.ajax({
                url: "GetDatesHandler.ashx", //该页面为处理信息页面，要求页面文件为空。（实际项目中建议用HttpHandler）
                type: "post",                //请求发送方式
                ...
                },   //传递的参数，传递过去后用 Request.Params["name1"]) 取出。
                //请求成功执行
        ...
        function choose() {
            $.ajax({
                url: "setActivity.aspx/Choose",
                type: "post",                //请求发送方式
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                data: "{tt:'" + $("#type option:selected").val() + "'}",//获取所选的下拉框
                //请求成功执行
                success: function (data, textStatus) {         
                    $("#menu_id").val(data.d);
