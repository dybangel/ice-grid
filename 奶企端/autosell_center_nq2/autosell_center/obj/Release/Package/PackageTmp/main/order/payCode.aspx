<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="payCode.aspx.cs" Inherits="autosell_center.main.order.payCode" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <div class="main">
            <div id="adminpopup" class="change zfpopup" style="top: 14%">
                <h4 id="title">添加机器设置</h4>
                <ul>
                    <li>
                        <h5>批次编号</h5>
                        <label>
                            <input id="pcbh" runat="server" class="input" readonly="true" />
                        </label>
                    </li>
                    <li>
                        <h5>卡片名称</h5>
                        <label>
                            <input id="cardName" runat="server" class="input" placeholder="" />
                        </label>
                    </li>
                    <li>
                        <h5>起始号</h5>
                        <label>
                            <input id="startNO" runat="server" class="input" placeholder="请输入12位数字不能以0开头" maxlength="12" />
                        </label>
                    </li>
                    ...
                    <li>
                        <h5>发行数量</h5>
                        <label>
                            <input id="cardNum" runat="server" class="input" type="text" />
                        </label>
                    </li>
                    <li>
                        <h5>单张面值</h5>
                        <label>
                            <input id="cardMoney" runat="server" class="input" type="text" />
                        </label>
                    </li>
                    <li>
                        <h5>开始时间</h5>
                        <label>
                            <input name="act_stop_timeks" type="text" id="startTime" runat="server" class="input" value="" placeholder="开始时间" />
                        </label>
                    </li>
                    <li>
                        <h5>结束时间</h5>
                        <label>
                            <input name="act_stop_timeks" type="text" id="endTime" runat="server" class="input" value="" placeholder="结束时间" />
                        </label>
                    </li>
                    <li>
                        <h5>登录密码</h5>
                        <label>
                            <input name="act_stop_timeks" type="text" id="loginPwd" runat="server" class="input" value="" placeholder="登录密码验证" />
                        </label>
                    </li>
                </ul>
                <dl>
                    <dd>
                        <input type="button" value="确定" class="popupqdbtn" onclick="saveAdd()" />
                    </dd>
                    <dd>
                ...
                <div class="common_main">
                    <div class="navlist">
                        <dl>
                            ...
                            <dd>
                                <a class="change acolor"  href="payCode.aspx"><i class="change icolor fa fa-window-restore"></i>充值卡列表</a>
                            </dd>
                        </dl>
                    </div>
                    <section class="jiqlist">
                        <dl class="jiqlistTab">
                            <dd class="change ddcolor"><b>充值卡列表</b></dd>
                        </dl>
                        <ul class="jiqlistseach">
                            ...
                            <li>
                                <input id="bh" runat="server" class="input" placeholder="批次、名称" />
                            </li>
                            <li>
                            ...
                            <li>
                                <input type="button" value="添加充值卡" class="seachbtn" onclick="addPayCode()" />
                            </li>
                        </ul>
...
<script>
    function judge() {
        $.ajax({
            type: "post",
            url: "payCode.aspx/judge",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: "{operaID:'" + $("#agentID").val() + "',menuID:'czklb'}",
    ...
    function saveAdd()
    {
        $.ajax({
            type: "post",
            url: "payCode.aspx/saveAdd",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: "{bh:'" + $("#pcbh").val() + "',name:'" + $("#cardName").val() + "',startNO:'" + $("#startNO").val() + "',cardNum:'" + $("#cardNum").val() + "',cardMoney:'" + $("#cardMoney").val() + "',startTime:'" + $("#startTime").val() + "',endTime:'" + $("#endTime").val() + "',operaID:'" + $("#agentID").val() + "',loginPwd:'" + $("#loginPwd").val() + "',companyID:'" + $("#companyID").val() + "'}",
            success: function (data) {
                if (data.d.code = "200") {
    ...
    function sear() {
        ...
        $(" <li>"
                   ...
                   + "<label style='width:10%'>起始号码</label>"
                   + "<label style='width:10%'>数量</label>"
                   + "<label style='width:10%'>单张面值</label>"
                   + "<label style='width:10%'>生成人</label>"
        ...
        $.ajax({
            type: "post",
            url: "payCode.aspx/sear",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: "{companyID:'" + $("#companyID").val() + "',startTime:'" + $("#start").val() + "',endTime:'" + $("#end").val() + "',keyword:'" + $("#bh").val() + "',statu:'" + $("#statuType").val() + "',pageCurrentCount:'" + $("#pageCurrentCount").val() + "'}",
            success: function (data) {
                if(data.d.code=="200")
                {
                    ...
                    for (var i = 0; i < serverdatalist; i++) {
                        ...
                        $(" <li>"
                                 ...
                                 + "   <span style='width:10%'>" + serverdata[i].startNO + "</span>"
                                 + "   <span style='width:10%'>" + serverdata[i].num + "</span>"
                                 + "   <span style='width:10%'>" + serverdata[i].mzMoney + "</span>"
                                 + "   <span style='width:10%'>" + (serverdata[i].operaName == null ? "管理员":serverdata[i].operaName) + "</span>"
                                 + "   <span style='width:10%'>" + serverdata[i].startTime + "</span>"
                                 + "   <span style='width:10%'>" + serverdata[i].endTime + "</span>"
                                 + "   <span style='width:10%'>" + (sta == "1" ? "正常" : (sta == "2" ? "禁用" : "")) + "</span>"
                                 + "   <span style='width:10%'><a onclick='setZF(\"" + serverdata[i].bh + "\")'>" + (sta == "1" ? "禁用" : (sta == "2" ? "正常" : "")) + "|</a><a href='payCodeList.aspx?bh=" + serverdata[i].bh + "'>详情</a></span>"
                                + "</li> ").appendTo("#ull");
                    }
                }
            }
