...
namespace Consumer.cls
{
    public class PayData
    {
        ...
        * @throws WxPayException
        */
        public SortedDictionary<string, object> FromXml(string xml,string companyID)
        {
            ...
            try
            {
                ...
                CheckSign(companyID);//验证签名,不通过会抛异常
            }
        ...
        */
        public string MakeSign(string companyID)
        {
            ...
            str += "&key=" + dt.Rows[0]["wx_partnerKey"].ToString();
            //MD5加密
            var md5 = MD5.Create();
        ...
        * 正确返回true，错误抛异常
        */
        public bool CheckSign(string companyID)
        {
            //如果没有设置签名，则跳过检测
            if (!IsSet("sign"))
            {
            ...
            //在本地计算新的签名
            string cal_sign = MakeSign(companyID);
