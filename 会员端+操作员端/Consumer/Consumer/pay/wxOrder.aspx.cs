...
{
    public partial class wxOrder : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (reqParams.ContainsKey("trxid"))
            {
                ...
                if (dtInfo.Rows.Count>0)
                {
                    ...
                    string sql1 = "select * from asm_orderlistDetail where orderNO in ('" + dtInfo.Rows[0]["orderNO"].ToString() + "') ORDER BY createTime DESC ";
                    DataTable d1 = DbHelperSQL.Query(sql1).Tables[0];
                    if (d1.Rows.Count>0)
                    {
                        string update12 = "UPDATE asm_orderlist set endTime='"+d1.Rows[0]["createTime"].ToString()+ "' WHERE orderNO in ('" + dtInfo.Rows[0]["orderNO"].ToString() + "')";
                        DbHelperSQL.ExecuteSql(update12);
                    }
                    if (dtInfo.Rows[0]["startTime"].ToString()==DateTime.Now.ToString("yyyy-MM-dd"))
                    {
                        string updagte = "UPDATE asm_orderlist set orderzt=1 where orderno='" + dtInfo.Rows[0]["orderNO"].ToString() + "'";
                        DbHelperSQL.ExecuteSql(updagte);
                    }
                    if (dtInfo.Rows[0]["source"].ToString() != "1") {
                        Util.growUpMemberBYDG(acct, int.Parse(dtInfo.Rows[0]["totalNum"].ToString()), dtInfo.Rows[0]["companyID"].ToString());
                    }
