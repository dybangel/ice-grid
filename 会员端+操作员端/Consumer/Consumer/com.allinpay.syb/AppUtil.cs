...
namespace uniondemo.com.allinpay.syb
{
    public class AppUtil
    {
        ...
        public static bool validSign(Dictionary<String, String> param, String appkey,String companyID)
        {
            string sql = "select * from asm_company where id="+companyID;
            DataTable dt = DbHelperSQL.Query(sql).Tables[0];
            String signRsp = param["sign"];
            param.Remove("sign");
            String sign = AppUtil.signParam(param, dt.Rows[0]["tl_APPKEY"].ToString());
            param.Add("key", appkey);
            String blankStr = BuildParamStr(param);
