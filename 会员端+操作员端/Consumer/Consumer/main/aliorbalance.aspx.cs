...
{
    public partial class aliorbalance : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    this.aLiopenID.Value = Request.QueryString["aLiopenID"].ToString();
                    ...
                    string sql2 = "select * from asm_member where aliOpenID='" + this.aLiopenID.Value+"'";
        ...
        public static string yzPwd(string unionID, string money, string companyID, string productID,string mechineID,string type,string dgOrderDetailID,string sftj)
        {
            ...
            string insertSQL = "insert into asm_pay_info(trxid,statu,type,payType,trxamt,createTime,appid,aliOpenID,mechineID,productID,companyID,orderType,dgOrderDetailID,sftj) values('" + trxid + "',0,2,4," + double.Parse(money) * 100 + ",'" + DateTime.Now + "','" + d2.Rows[0]["tl_APPID"].ToString() + "','" + unionID+ "','"+ mechineID + "','"+ productID + "','"+ companyID + "','"+type+"','"+dgOrderDetailID+"','"+ sftj + "')";
            Util.Debuglog("insertSQL="+ insertSQL, "余额支付.txt");
            int a = DbHelperSQL.ExecuteSql(insertSQL);
    

            //验证密码是否正确
            string sql = "select * from asm_member where aliOpenID='" + unionID + "'";
            Util.Debuglog("sql=" + sql, "余额支付.txt");
            DataTable dd = DbHelperSQL.Query(sql).Tables[0];

           
            string openID = dd.Rows[0]["aliOpenID"].ToString();
            //判断该订单支付状态
            ...
            //更新余额
            string update = "update asm_member set AvailableMoney=AvailableMoney-" + money + ",consumeCount=consumeCount+1,sumConsume=sumConsume+" + money + ",LastTime='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where aliOpenID='" + openID + "'";
            ...
            string update1 = "update asm_pay_info set chLdNo='"+ldno+"', trxdate='" + DateTime.Now.ToString("yyyyMMdd")+"',statu=1,paytime='" + DateTime.Now.ToString("yyyyMMddHHmmss") + "',acct='" + openID + "',afterMoney="+(double.Parse(dd.Rows[0]["AvailableMoney"].ToString())-double.Parse(money))+" where trxid='" + trxid + "' and aliOpenID='" + unionID + "'";
