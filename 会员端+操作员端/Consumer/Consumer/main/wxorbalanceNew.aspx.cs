...
{
    public partial class wxorbalanceNew : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    this._reqsn.Value = Request.QueryString["reqsn"].ToString();//交易单号
                    ...
                    if (activityiddt.Rows.Count > 0)
                    {
                        ...
                        string discountsql = "select b.* ,(select count(1) from asm_pay_info a "
                                            + "where a.unionid = '" + this._unionID.Value + "' and a.type = 2 and a.statu = 1  and a.productid> ''  and a.createTime>=b.startTime and a.createTime<=b.endTime and  orderType<>3 "
                    ...
                    //转售金额固定
                    if (this._dgOrderID.Value != "0" && !string.IsNullOrEmpty(this._dgOrderID.Value))
                    {
                        string sqlDetail = "select * from asm_orderlistDetail where id=" + this._dgOrderID.Value;
                        DataTable dtDetail = DbHelperSQL.Query(sqlDetail).Tables[0];
                        if (dtDetail.Rows.Count > 0)
                        {
                            
                            price = double.Parse(dtDetail.Rows[0]["sellPrice"].ToString()).ToString("0.00");
                            afterMoney = dtDetail.Rows[0]["sellPrice"].ToString();
                        }
        ...
        {
            //判断该订单支付状态
            string sql1 = "select * from asm_product_pick where reqsnNo='" + reqsn + "' ";
            Util.Debuglog("sql1=" + sql1, "余额支付.txt");
            DataTable d1 = DbHelperSQL.Query(sql1).Tables[0];
            if (d1.Rows.Count > 0 && d1.Rows[0]["payStatus"].ToString() == "3")
            {
                return "4";//已经支付完成无需重复支付
            }
            string pickupdate = "update  asm_product_pick set payStatus=2,paytype='3',startPayTime='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where reqsnNo='" + reqsn + "' ";
            ...
            pickupdate = "update  asm_product_pick set payStatus=3,payTime='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where reqsnNo='" + reqsn + "' ";
            Util.Debuglog("sqlInsert=" + pickupdate, "获取预生成订单号.txt");
            DbHelperSQL.ExecuteSql(pickupdate);
            
            string searchSql = "select 1 from asm_firstPayRecord where mechineID=" + mechineID + " AND memberID='" + dd.Rows[0]["id"].ToString() + "'";
            ...
            if (searchSqldt.Rows.Count > 0)
            {
                ...
                string insertsql = "insert into   asm_firstPayRecord(mechineID,memberID,firstinfo,firstbuyTime,type) values (" + mechineID + "," + dd.Rows[0]["id"].ToString() + ",'"+ trxid + "','"+ DateTime.Now.ToString("yyyy-MM-dd") + "','"+2+"') ";
            ...
            Util.chNew(ldno, dM.Rows[0]["id"].ToString(), trxid, "3", productID, money);
