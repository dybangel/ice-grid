...
import com.sq.util.ShellUtils.CommandResult;
...
public class PackageUtils {
	 ...
	 * @param context
	 * @param filePath file path of package
	 * @return {@link PackageUtils#INSTALL_SUCCEEDED} means install success, other
	 *         means failed. details see {@link PackageUtils}.INSTALL_FAILED_*. same
	...
	public static int uninstallSilent(Context context, String packageName, boolean isKeepData) {
		...
		Log.i("3333", "pack2"+command.toString());
		CommandResult commandResult = ShellUtils.execCommand(command.toString(), !isSystemApplication(context), true);
		Log.i("3333", "pack3");
		if (commandResult.successMsg != null
				&& (commandResult.successMsg.contains("Success") || commandResult.successMsg.contains("success"))) {
	 ...
	 * @param context
	 * @param packageName
	 * @return if params error or task stack is null, return null, otherwise retun
	 *         whether the app is on the top of stack
	 */
	 ...
	 * package and the caller has not supplied the {@link #INSTALL_ALLOW_TEST} flag.
	 */
	...
	/**
	 * Uninstall return code<br/>
	 * uninstall fail if the system failed to delete the package for an unspecified
	 * reason.
	 */
