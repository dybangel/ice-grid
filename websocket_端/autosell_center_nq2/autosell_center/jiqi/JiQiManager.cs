...
{
    public class JiQiManager
    {
        public static  ConcurrentDictionary<String, JiQi> list = new ConcurrentDictionary<string, JiQi>();

        public static void Init()
        {

            DataTable da = RedisUtil.getAllMechine();
            if (da != null && da.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < da.Rows.Count; i++)
                {
                    ...
                    mid = da.Rows[i]["id"].ToString();
                    JArray jay = RedisUtil.DeserializeObject(RedisUtil.getMechine(mid));
                    if (jay != null)
                    {
                        string jiqiName = jay[0]["mechineName"].ToString();
                        string status = jay[0]["zt"].ToString();
                        JiQi jiqi = new JiQi(mid, jiqiName,status);
                        Set(mid, jiqi);
                    }
                }
            }
        }
        public static void Set(string key, JiQi value)
        {
            list.AddOrUpdate(key, value, (k, o) => value);
        }

        public static JiQi Get(string key)
        {
            JiQi entry;
            if (!list.TryGetValue(key, out entry))
            {
                return null;
            }
            return entry;
