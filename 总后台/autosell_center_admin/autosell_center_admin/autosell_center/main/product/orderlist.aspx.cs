    ...
    public partial class orderlist : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //this.companyList2.DataSource = dt2;
                //this.companyList2.DataBind();
                //this.companyList2.Items.Insert(0, new ListItem("所有机器", "0")); //添加项             
            }
        }
        ...
        //        this.companyList2.DataBind();
        //        this.companyList2.Items.Insert(0, new ListItem("所有机器", "0")); //添加项
        //        this.companyList2.SelectedValue = "0";       
        //    }
        //}
        //[WebMethod]
        ...
        public static object getMechineList(string companyID)
        {
            try
            {
                ...
                return new { code = 200, msg = "请先选择企业" , db =""};
            }
            catch
            {
        ...
        {
            if (this.companyList.SelectedValue.Equals("0")) {
                Response.Write("<script>alert('请先选择企业!');</script>");
                return;
            }
            string sql = " 1=1 ";
            ...
            {
                string brandSql = "SELECT  brandID ,value = ( STUFF(( SELECT    ',' + convert(varchar,productID) FROM asm_product"
                         + " WHERE brandID = Test.brandID  FOR XML PATH('') ), 1, 1, '') )FROM asm_product  AS Test where companyID = " + this.companyList.SelectedValue + " and brandID in(" + this._brandList.Value + ") GROUP BY brandID; ";
                DataTable brandDt = DbHelperSQL.Query(brandSql).Tables[0];
                if (brandDt.Rows.Count > 0)
                {
                    sql += " and productID in(" + brandDt.Rows[0]["value"].ToString() + ")";
