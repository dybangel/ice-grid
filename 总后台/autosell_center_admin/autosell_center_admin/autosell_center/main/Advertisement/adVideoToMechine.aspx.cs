...
namespace autosell_center.main.Advertisement
{
    public partial class adVideoToMechine : System.Web.UI.Page
    {
        ...
        {
            string sql = @"select *,name + '   ' +case type when '0' then '横屏' when '1' then '竖屏' else '其他' end name from asm_video where id not in (select videoID from asm_videoAddMechine where mechineID = "+mechineID+")";
        ...
        public static string okbtn(string id, string mechine_ID)
        {
            if (id.Trim() != "")
            {
                ...
                {
                    string sql12 = "select * from asm_videoAddMechine where mechineID=" + mechine_ID + " and videoID=" + idArr[i];
                    ...
                    if (d12.Rows.Count<=0)
                    {
                        ...
                        string sql = "insert into asm_videoAddMechine (mechineID,videoID,type,tfType,tfcs,tfTime) values(" + mechine_ID + "," + idArr[i] + "," + dt.Rows[0]["type"].ToString() + ",1,1000,'"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"')";
