...
namespace autosell_center.main.Big_screen
{
    public partial class big_screen : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string sql = "select id,bh,zdX,zdY from asm_mechine where zdX!='' and zdY!='' and statu=0";
            ...
            string sql1 = "select id,bh,zdX,zdY from asm_mechine where zdX!='' and zdY!='' and statu!=0";
        ...
        {
            string sql = "select count(*) orderNum,sum(totalMoney) totalMoney from asm_sellDetail   where type=2 and bz!='退款成功'";
            DataTable dt = DbHelperSQL.Query(sql).Tables[0];
             
            string sql3 = "select count(*) orderNum,sum(totalMoney) totalMoney from asm_order ";
            DataTable dt3 = DbHelperSQL.Query(sql3).Tables[0];

            string sql4 = "select count(*) orderNum,isnull(sum(totalMoney),0) totalMoney from asm_sellDetail where bz!='退款成功'";
            DataTable dt4 = DbHelperSQL.Query(sql4).Tables[0];
            int totalOrderNum = int.Parse(dt3.Rows[0]["orderNum"].ToString());
        ...
        public static string getOrderList()
        {
            string sql = @"select A.*,ac.name,'1' type,''bz from (select am.bh,am.companyID,ao.createTime from asm_order ao left join asm_mechine am on ao.mechineID=am.id  where createTime like '" + DateTime.Now.ToString("yyyy-MM-dd")+"%') A left join asm_company ac on A.companyID=ac.id union all"
                 + "   select B.bh,B.companyID,CONVERT(varchar(100), convert(datetime,B.orderTime), 20),ab.name,'2',bz from (select am.bh,am.companyID,asd.orderTime,asd.bz from asm_sellDetail asd left join asm_mechine am on asd.mechineID=am.id where CONVERT(varchar(100), convert(datetime,orderTime), 20) like '" + DateTime.Now.ToString("yyyy-MM-dd")+"%') B left join asm_company ab on B.companyID=ab.id ";
            DataTable dt = DbHelperSQL.Query(sql).Tables[0];
            if (dt.Rows.Count>0)
            {
        ...
        /// <returns></returns>
        [WebMethod]
        public static string mechineList(string type)
        {
            if (type == "1")
            {
                ...
                if (dd.Rows.Count > 0)
                {
                    ...
                    string sql2 = "select (select name from asm_company where asm_company.id=am.companyID) companyName,* from asm_mechine am where id in (" + id + ")";
        ...
        [WebMethod]
        public static string ssts()
        {
            string sql = @"select acct,am.name,am.sumConsume,am.sumRecharge,am.AvailableMoney, SUBSTRING(paytime,0,5)+'-'+SUBSTRING(paytime,5,2)+'-'+SUBSTRING(paytime,7,2)+' ' +SUBSTRING(paytime,9,2)+':'+SUBSTRING(paytime,11,2) time from asm_pay_info left join asm_member am
                    on asm_pay_info.acct = am.openID
                    where 1 = 1 and type = 1 and paytime like  '"+DateTime.Now.ToString("yyyyMMdd")+"'   order by paytime desc";
            DataTable dt = DbHelperSQL.Query(sql).Tables[0];
            if (dt.Rows.Count > 0)
            {
                string ss = OperUtil.DataTableToJsonWithJsonNet(dt);
